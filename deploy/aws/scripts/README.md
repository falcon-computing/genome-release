...
- Configure aws such that message can be sent out.

- Storage Device should be defined in /local/ 
  Two sub-folders need to be created: /local/fastq/ and /local/ref/
  
- /local/ref/ can be populated from the folder /genome/ref/
  cp /genome/ref/human_g1k_v37.* . & 
  cp /genome/ref/*vcf*  . & 

- /local/fastq/ can be populated using data posted
