...
1. 申请一台32核ECS云服务器（例如: m2.8xlarge.8），和一台FP1云服务器。两台云服务器均选择峰科基因镜像，且需要放在同一个可用区。请记录FP1云服务器的私有IP（例如：192.168.x.x)。
1. （可选）挂载云盘，峰科加速基因软件对本地存储有较高存取吞吐率要求，建议使用超高IO本地云盘存储输入输出和临时文件以最大化分析性能。
1. 确认临时文件存储目录，并确保临时目录有足够可用空间，一般情况下，峰科加速软件运行过程中会产生临时文件，大小为输入*FASTQ.GZ*文件的3到5倍。临时目录需要对ECS云服务器和FP1服务器共享，
...
# 配置步骤
镜像配置可通过运行自动配置脚本完成，此脚本包含峰科基因软件所需临时目录配置以及参考基因预处理配置。登陆镜像后，按如下命令运行配置脚本：
```
...
1. 配置SSH，需要输入FP1的私有IP。假设IP是*192.168.1.2*，配置界面如下：
    ...
    ```
1. 配置临时文件夹。如果用户已配置好共享存储，此步可直接提供存储路径；如果还未配置共享存储，用户需提供云盘位置（如/dev/vdb），此脚本会初始化并加载本地云盘。脚本提示如下：
    ```
