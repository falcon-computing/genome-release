...
Once the alignment is completed,  indel-realignment can be perfomed.  The BASH script below demonstrates the usage of the indel method:
```
REF="/local/ref/human_g1k_v37.fasta
SAMPLE_ID="NA12878"
...
This feature performs by default two GATK processes:  RealignerTargetCreator and IndelRealigner. The indel interval list
generated by RealignerTargetCreator is not generated as an output by this feature. Instead, it generates directly the output folder NA12878_marked_sorted_indel_realign/ that contains a set of BAM and bai files with indels re-aligned. If user has an interval list file and desire to use it, the the option -L is available for that purpose. 
