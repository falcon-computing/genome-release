...
@test "JOINT input directory does not exists but input file does not exist" {
   run ${FCSBIN} joint -r ${ref_genome} -i fake/ -o out.vcf
   [ "$status" -ne 0 ]
   [[ "${output}" == *"ERROR"* ]]
}

@test "JOINT database name set but not defined" {
   run ${FCSBIN} joint -r ${ref_genome} -i ${INPUT_DIR} -o test.vcf --database_name  --gatk4
   [ "$status" -ne 0 ]
   [[ "${output}" == *"ERROR"* ]]
}

@test "JOINT sample id set but not defined" {
   run ${FCSBIN} joint -r ${ref_genome} -i ${INPUT_DIR} -o out.vcf --database_name my_database --sample-id  --gatk4
   [ "$status" -ne 0 ]
   [[ "${output}" == *"ERROR"* ]]
}
